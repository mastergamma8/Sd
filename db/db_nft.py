...
async def get_nft_stars(user_id: int) -> int:
    async with aiosqlite.connect(DB_NAME) as db:
        row = await db.fetchrow(
            "SELECT nft_stars FROM users WHERE tg_id = $1", user_id
        )
...
async def add_nft_stars(user_id: int, amount: int) -> int:
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        row = await db.fetchrow(
            """
            UPDATE users SET nft_stars = nft_stars + $1
            WHERE tg_id = $2
            RETURNING nft_stars
            """,
            amount, user_id
...
async def spend_nft_stars(user_id: int, amount: int) -> tuple[bool, int]:
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        row = await db.fetchrow(
            "SELECT nft_stars FROM users WHERE tg_id = $1 FOR UPDATE", user_id
        )
        ...
        await db.commit()
        return True, int(updated["nft_stars"])
...
async def get_painting(painting_id: int) -> dict | None:
    async with aiosqlite.connect(DB_NAME) as db:
        row = await db.fetchrow(
            "SELECT * FROM nft_paintings WHERE id = $1", painting_id
        )
        if not row:
            return None
        return dict(row)
...
async def toggle_painting(painting_id: int, is_active: bool) -> bool:
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
            "UPDATE nft_paintings SET is_active = $1 WHERE id = $2",
...
async def delete_painting(painting_id: int) -> bool:
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute("DELETE FROM nft_paintings WHERE id = $1", painting_id)
        await db.commit()
        return True
...
async def buy_painting(user_id: int, painting_id: int) -> tuple[bool, str]:
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        exists = await db.fetchrow(
            "SELECT id FROM nft_owned WHERE user_id = $1 AND painting_id = $2",
            user_id, painting_id
        )
        if exists:
            return False, "already_owned"

        # Берём картину с блокировкой
        painting = await db.fetchrow(
            "SELECT * FROM nft_paintings WHERE id = $1 FOR UPDATE", painting_id
        )
        if not painting or not painting["is_active"]:
            return False, "not_found"

        # Проверяем лимит выпуска
        if painting["total_supply"] and painting["total_supply"] > 0:
            if painting["sold_count"] >= painting["total_supply"]:
                return False, "sold_out"

        # Проверяем и списываем NFT-звёзды
        ...
        # Списываем
        await db.execute(
            "UPDATE users SET nft_stars = nft_stars - $1 WHERE tg_id = $2",
            price, user_id
        )
        # Добавляем в коллекцию
        await db.execute(
            "INSERT INTO nft_owned (user_id, painting_id) VALUES ($1, $2)",
            user_id, painting_id
        )
        # Увеличиваем счётчик продаж
        await db.execute(
            "UPDATE nft_paintings SET sold_count = sold_count + 1 WHERE id = $1",
            painting_id
        )
        await db.commit()
        return True, "ok"
